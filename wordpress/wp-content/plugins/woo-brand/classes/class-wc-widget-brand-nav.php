<?php 
register_widget( 'woo_brands_layered_nav_init' );
class woo_brands_layered_nav_init extends WC_Widget {
	...
	protected function get_page_base_url( $taxonomy ) {
		if(version_compare(WC()->version, '3.0.0', '>=')){
			$wc_get_3='wc_get_page_id';
		}
		else
		{
			$wc_get_3='woocommerc_get_page_id';
		}
		
		if ( defined( 'SHOP_IS_ON_FRONT' ) ) {
			$link = home_url();
		} elseif ( is_post_type_archive( 'product' ) || is_page( $wc_get_3( 'shop' ) ) ) {
