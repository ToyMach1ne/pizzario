...
function dhvc_woo_explode(delimiter, string, limit) {
...
(function($) {
    tinymce.create('tinymce.plugins.dhvcwoocommerce', {
       init : function(el, url) {
    	   el.addButton('dhvcwoocommerce_button', {
                title : 'DHVC Wooommerce Shortcode',
                image : dhvcwoocommerce.plugin_url + '/assets/images/icon.png',
                onclick: function() {
                    var width = $(window).width(), H = $(window).height(), W = (720 < width) ? 720 : width;
                    W = W - 80;
                    H = H - 115;
                    tb_show('DHVC Wooommerce Shortcode', '#TB_inline?width=' + W + '&height=' + H + '&inlineId=dhvcwoocommerce_form');
                }
            });
        },
    });
    tinymce.PluginManager.add('dhvcwoocommerce', tinymce.plugins.dhvcwoocommerce);
   
    
    $(document).ready(function(){
    	 var form = $('<div id="dhvcwoocommerce_form">'+dhvcwoocommerce.form+'</div>');
    	 form.appendTo('body').hide();
    	    
    	$('.dhvc-woo-color-control').wpColorPicker();
    	$('#dhvcwoocommerce-shortcode-submit').click(function(e){
    		 var shortcode = '[dhvc_woo_products ';
    		 $('.dhvc-woo-shortcode-params input,.dhvc-woo-shortcode-params textarea,.dhvc-woo-shortcode-params select').each(function(){
    			 if($(this).hasClass('dhvc-woo-param-value')){
	    			 if($(this).is('input[type="text"]') && $(this).val() != ''){
	    				 ...
	    				 shortcode += ' '+ $(this).attr('name') + '="'+$(this).find('option:selected').attr('value')+'"';
	    			 }
    			 }
    		 });
    		 shortcode +=']';
    	...
    	$('[data-dependency="toggle"]').closest('.dhvc-woo-shortcode-param').hide();
    	
    	var $this = $('.dhvc-woo-shortcode-params select#query_type'),val = $this.val();
		$('[data-dependency-element="'+$this.attr('id')+'"]').each(function(){
  			var dependency_value = dhvc_woo_explode(',',$(this).data('dependency-value'));
			if($.inArray(val+'',dependency_value) > -1){
				$(this).closest('.dhvc-woo-shortcode-param').show();
			}else{
				$(this).closest('.dhvc-woo-shortcode-param').hide();
			}
		});
    	
    	$('.dhvc-woo-shortcode-params select#query_type').on('change',function(){
    	...
    	var $this = $('.dhvc-woo-shortcode-params  select#display'),val = $this.val();
		$('[data-dependency-element="'+$this.attr('id')+'"]').each(function(){
  			var dependency_value = dhvc_woo_explode(',',$(this).data('dependency-value'));
			if($.inArray(val+'',dependency_value) > -1){
				$(this).closest('.dhvc-woo-shortcode-param').show();
    	...
    	$('.dhvc-woo-shortcode-params  select#display').on('change',function(){
    		var $this = $(this),val = $this.val();
   		 	$('[data-dependency-element="'+$this.attr('id')+'"]').each(function(){
      			var dependency_value = dhvc_woo_explode(',',$(this).data('dependency-value'));
    			if($.inArray(val+'',dependency_value) > -1){
		...
		$('.dhvc-woo-select').on('change',function(){
			var v = $(this).val(),
				id = $(this).attr('id');
		...
		$('.dhvc_ajax_products').ajaxChosen({
		    method: 	'GET',
		    url: 		ajaxurl,
		    dataType: 	'json',
		    afterTypeDelay: 100,
		    data:		{
		    	action: 		'dhvc_woo_search_products'
		    }
		}, function (data) {
			var terms = {};
