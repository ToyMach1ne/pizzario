        ...
        public function get_categories_list_html( $args, $terms, $taxonomy, $display_type, $query_type, $instance, $terms_type_list, $current_term, $categories_filter_operator, $is_chosen_class, $is_parent_class, $is_child_class, $level = 0 ){
            ...
            foreach ( $terms as $parent_id => $term_ids ) {
                ...
                if ( $query_type == 'and' ) {
                    ...
                    if ( apply_filters( 'yith_wcan_skip_no_products_in_category', ( ! yit_term_has_child( $term, $taxonomy ) ) && $count == 0 && ! $option_is_set ) ) {
                        continue;
                    }
...
//                    if( ! $in_array_function( $queried_object->slug, $current_filter ) ){
//                        $link = add_query_arg( array( 'source_id' => $queried_object->term_id,  ), $link );
//                    }
                }
                ...
                if( ! apply_filters( 'yith_wcan_skip_current_categories', $skip, $taxonomy, $terms, $count ) ){
                    ...
                    elseif( ! apply_filters( 'yith_wcan_skip_no_products_in_category', $filter_is_hierarchical  ) ) {
                        printf( '<span>%s</span>', $term->name );
                    }

                    if ( apply_filters( 'yith_wcan_force_show_count_in_category', $count != 0 ) && apply_filters( "{$args['widget_id']}-show_product_count", true, $instance ) ) {
