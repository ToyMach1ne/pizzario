	...
	class YITH_YWPI_Template {
		...
		public function print_notice_preview_mode( $text ) {
			...
			return sprintf ( "<span class=\"document-type\">%s</span><br>%s",
				__ ( 'THIS IS ONLY A PREVIEW DOCUMENT', 'yith-woocommerce-pdf-invoice' ),
				$text );
		}
		...
		public function show_company_data( $document ) {
			...
			//$company_logo_path = $_SERVER['DOCUMENT_ROOT'] . preg_replace( "!http(s)?://" . $_SERVER['SERVER_NAME'] . "/!", "/", $company_logo );
		...
		public function show_customer_details( $document ) {
			...
			if ( ywpi_document_behave_as_invoice ( $document ) ) {
				...
				$content = wp_kses ( YITH_PDF_Invoice ()->get_customer_billing_details ( $order_id ), array( "br" => array() ) );
			} else {
				$content = wp_kses ( YITH_PDF_Invoice ()->get_customer_shipping_details ( $order_id ), array( "br" => array() ) );
			}
