...
if ( ! class_exists ( 'YITH_Invoice_Details' ) ) {
	...
	class YITH_Invoice_Details {
		 ...
		 * @since  1.0.0
		 */
		public function get_variation_text( $item_id ) {
			$variation_text = '';
			...
			if ( ywpi_is_enabled_column_variation ( $this->document ) ) {
				...
				if ( $metadata ) {
					...
					foreach ( $metadata as $meta ) {
						...
						$variation_text .= apply_filters ( 'yith_ywpi_template_product_variation_string', sprintf ( '%s: %s ', $meta['meta_key'], $meta['meta_value'] ), $meta );
					}
				}
			}
		...
		public function get_order_subtotal( $incl_order_discount = true ) {
			...
			$_order_subtotal = $this->order->get_subtotal () +
			                   $this->order->get_total_shipping () +
			                   $order_fee_amount;
			
			if ( $incl_order_discount ) {
				$_order_subtotal += $this->get_products_total_discount ();
			}
		...
		public function get_order_total() {
			...
			$_order_total = apply_filters ( 'yith_ywpi_invoice_total',
				$this->get_order_subtotal () - $this->get_order_discount () + $_order_taxes_total, yit_get_prop ( $this->order, 'id' ) );
		...
		public function get_order_discount() {
			$_order_discount = apply_filters ( 'yith_ywpi_invoice_total_discount',
				$this->order->get_total_discount () + $this->get_products_total_discount (),
				$this->order,
				$this->get_products_total_discount () );
